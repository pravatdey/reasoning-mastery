"""
Regenerate only the audio using Gemini TTS, then recompose the video.
Reuses existing script — NO Groq API call. Only Gemini TTS call.

Usage: python regenerate_audio.py --part 1
"""
...
async def main():
    parser = argparse.ArgumentParser(description="Regenerate audio + recompose video")
    parser.add_argument("--part", type=int, required=True, help="Part number")
    args = parser.parse_args()

    setup_logger(log_file="logs/regenerate.log")
    ...
    # Step 1: Regenerate audio with Gemini TTS
    logger.info("Generating audio with Gemini TTS...")
    tts = TTSManager("config/settings.yaml")
    ...
    logger.info(f"Audio generated: {result.duration:.1f}s, provider={result.voice.provider}")

    # Step 2: Recompose video
    logger.info("Composing video with intro...")
    ...
    print(f"\nDone!")
    print(f"Audio: {audio_path} ({result.duration:.1f}s, {result.voice.provider})")
